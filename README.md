...
| | |
|---|---|
| `POST /v1/threads` | Start a thread. The server mints the id, the caller cannot choose it |
...
The applicant side has no login, deliberately, so a thread id is the only thing
guarding a thread. That makes it a bearer capability, and until recently the
caller chose it: `amara` was a valid id. The server mints it now, 256 bits from
`secrets`, with nothing about the person in it.
