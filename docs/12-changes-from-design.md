...
## 28. A thread id was the caller's to choose, and it is the credential

`05-LLD.md` has `POST /v1/threads` take a `thread_id` from the body. Every
example in this repository used names like `amara`, and so did the tests.

That is fine right up until you notice what a thread id *is*. Applicants have no
account, deliberately: PDD section 4 is about somebody in an emergency
accommodation queue, and requiring registration before they can find out where
the nearest GP is defeats the project. Nothing else guards the thread. So the id
is a bearer capability, and **a capability the caller chooses is a capability
anybody can guess**. `amara` was a valid id.

The fix is small and the reasoning is the same shape as change 27: the thing
that authenticates must not be supplied by the party being authenticated. The
server mints the id now, 256 bits from `secrets`, and `extra="forbid"` refuses a
body that still sends one rather than ignoring it and leaving somebody believing
they chose an id that was really something else. Nothing about the person goes
into the id, and a test asserts it: an id derived from a name or an arrival date
would be a capability that leaks exactly what it protects, and for this
population that is the category of data whose disclosure can reach the
authorities somebody left.

The response says so in words the client will actually read:

    "This id is how you get back to your plan, and anybody who has it can
     read it. Keep it like a password."

**What this does not fix**, and what no amount of entropy would: a capability
that leaks is a capability somebody else holds, and the server cannot tell the
difference. Keeping the id out of referrer headers, logs and shared screens is a
deployment concern, and `DELETE /v1/threads/{id}` is the revocation. The gap
that closed is guessing, not sharing.
